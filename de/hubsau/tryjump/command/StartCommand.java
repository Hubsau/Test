			...
			if (GameStateManager.getGameState() instanceof LobbyState) {
				...
				if (seconds >= 5) {
					if (lobbyState.getCountdown().isRunning()) {
						...
						player.sendMessage(prefix + "§2Du hast den Lobbycountdown beschl§unigt");
					} else {
						player.sendMessage(prefix
								+ "§2Du hast die Z§hlzeit des Lobbycountowns f§r diese Runde auf 5 Sekunden gesetzt");
						lobbyState.getCountdown().setSeconds(5);

					}
				} else
					player.sendMessage(
							prefix + "§cDer Countdown kann nur beschl§unigt werden wenn er §ber 5 Sekunden ist");

			} else {
				player.sendMessage(prefix
						+ "§cDas Spiel hat bereits begonnen du kannst deswegen den Countdown auch nicht mehr beschl§unigen");
			}
